...
def plot_confusion_matrix(target_list = None, labels_val = None, labels_val_predicted= None,cf_matrix=None, Get_matrix=False) -> None:
    """
    ...
    labels = np.asarray(labels).reshape(len(target_list), len(target_list))

    fig, ax = plt.subplots(figsize=(15, 10))
    sns.set(font_scale=1.2)
    sns.heatmap(annotations, annot=labels, fmt='', cmap='Blues', cbar=True, xticklabels=target_list, yticklabels=target_list)
...
def model_metrics(target_list, labels_val, labels_val_predicted, Get_metrics=False): 
    """
    ...
    """
    print(classification_report(labels_val, labels_val_predicted, target_names=[str(x) for x in target_list]))
